...
/*
  ...
  uses a database of words categorised as positiveitive, negativeative or neutraltral
...
class Sentiment {
	...
	/**
	 * List of words with negativeative prefixes, e.g. isn't, arent't
	 * @var array
	 */
	private $negativePrefixList = array();
	...
	public function score($sentence) {
		...
		//For each negativeative prefix in the list
		foreach ($this->negativePrefixList as $negativePrefix) {

			//Search if that prefix is in the document
			if (strpos($sentence, $negativePrefix) !== false) {
				//Reove the white space after the negativeative prefix
				$sentence = str_replace($negativePrefix . ' ', $negativePrefix, $sentence);
			}
		}
		...
		//Empty array for the scores for each of the positivesible categories
		$scores = array();

		//Loop through all of the different classes set in the $classes variable
		foreach ($this->classes as $class) {

			//In the scores array add another dimention for the class and set it's value to 1. EG $scores->negative->1
			$scores[$class] = 1;
	 ...
	 * 
	 * @param str $sentence
	 * @return str positive|neutral|negative
	 */
	...
	private function loadDefaults(){
		...
		//Get the list of negativeative prefixes
		$this->negativePrefixList = $this->getList('prefix');

		//If negative prefix list not set give error
		if (!isset($this->negativePrefixList))
			echo 'Error: Ignore List not set';
	}
